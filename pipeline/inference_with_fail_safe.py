"""
inference_with_fail_safe.py
---------------------------
Run single-file inference with a fail-safe fallback for low-confidence outputs.

If the model's best main-class or subclass confidence stays below the configured
threshold, the script marks the result as unknown / unreliable instead of
returning a normal class prediction.

Usage:
    python pipeline/inference_with_fail_safe.py --audio path/to/file.wav
    python pipeline/inference_with_fail_safe.py --audio file.wav --fail-safe-threshold 0.45
    python pipeline/inference_with_fail_safe.py --audio file.wav --json
"""
...
UNKNOWN_MESSAGE = "unable to get reliable prediction"
...
def predict_audio(
    model: AcousticCRNN,
    spec: torch.Tensor,
    fail_safe_threshold: float = 0.45,
    top_k: int = 3,
) -> dict:
    """Run inference and convert uncertain predictions into unknown."""
    ...
    low_confidence = (
        float(main_conf.item()) < fail_safe_threshold
        or float(sub_conf.item()) < fail_safe_threshold
    )

    predicted_main = cfg.IDX_TO_MAIN[main_idx.item()]
    predicted_sub = cfg.IDX_TO_SUB[sub_idx.item()]

    return {
        "main_class": UNKNOWN_LABEL if low_confidence else predicted_main,
        "main_confidence": float(main_conf.item()),
        "subclass": UNKNOWN_LABEL if low_confidence else predicted_sub,
        "sub_confidence": float(sub_conf.item()),
        "fail_safe_triggered": low_confidence,
        "fail_safe_threshold": fail_safe_threshold,
        "status_message": UNKNOWN_MESSAGE if low_confidence else "prediction accepted",
        "raw_prediction": {
            "main_class": predicted_main,
            "subclass": predicted_sub,
        },
...
def print_prediction(audio_path: Path, checkpoint: dict, result: dict) -> None:
    ...
    print("=" * 72)
    print("  ACOUSTIC CRNN INFERENCE WITH FAIL-SAFE")
    print("=" * 72)
    print(f"  Audio file:             {audio_path}")
    print(f"  Device:                 {cfg.DEVICE}")
    print(f"  Checkpoint epoch:       {checkpoint.get('epoch', 'unknown')}")
    print(f"  Fail-safe threshold:    {result['fail_safe_threshold']:.2f}")
    ...
    print()
    print("Prediction")
    print(f"  Main class:             {result['main_class']:<20} ({result['main_confidence']:.4f})")
    print(f"  Subclass:               {result['subclass']:<20} ({result['sub_confidence']:.4f})")
    print(f"  Fail-safe triggered:    {result['fail_safe_triggered']}")
    print(f"  Status:                 {result['status_message']}")

    if result["fail_safe_triggered"]:
        print()
        print("Raw Best Prediction Before Fail-Safe")
        print(f"  Main class:             {result['raw_prediction']['main_class']}")
        print(f"  Subclass:               {result['raw_prediction']['subclass']}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run single-file inference with a low-confidence fail-safe."
    )
    parser.add_argument(
        "--audio",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.45,
        help="If the best main or subclass confidence is below this, predict unknown.",
    )
    parser.add_argument(
        "--top-k",
        type=int,
    ...
    result = predict_audio(
        model,
        spec,
        fail_safe_threshold=args.fail_safe_threshold,
        top_k=args.top_k,
    )
